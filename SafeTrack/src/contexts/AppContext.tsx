...
export function AppProvider({ children }: AppProviderProps) {
  ...
  const initializeApp = async () => {
    try {
      ...
      // In a real app, this would be handled by user authentication
      let user = await dbService.getUserById(1);
      ...
      if (!user) {
        ...
        const defaultThresholds = AlertEngine.getDefaultThresholds(userId, ['hypertension']);
